...
(
  ...
  cd nise_bosh
  sudo env PATH=$PATH bundle exec ./bin/nise-bosh --keep-monit-files -y ../cf-release ../manifests/nise-diego-cf-components.yml cf_components_diego -n ${NISE_IP_ADDRESS}
  sudo env PATH=$PATH bundle exec ./bin/nise-bosh --keep-monit-files -y ../diego-release ../manifests/nise-diego.yml diego -n ${NISE_IP_ADDRESS}
  sudo env PATH=$PATH bundle exec ./bin/nise-bosh --keep-monit-files -y -r ../etcd-release/releases/etcd/etcd-${ETCD_VERSION#v}.yml ../etcd-release ../manifests/nise-diego-etcd.yml etcd -n ${NISE_IP_ADDRESS}
  ...
  find /root/etcd-release/.final_builds/packages/*/ -type f -iname "*.tgz" -print0 | xargs -0 -I {} truncate {} --size 0
)
